...
contract DAO is IWormholeReceiver {
    ...
    function proposal(
        ...
        require(expiration > block.number, "expires before voting ends");
        ...
        DecodedProposal memory dp = DecodedProposal(
            proposalCount,
            description,
            // Note we use blocknumber - 1 here as a flash loan mitigation.
            uint128(block.number - 1),
        ...
        emit ProposalCreated(proposalCount, block.number, expiration);
    ...
    function receiveWormholeMessages(
        ...
        ProposalResult memory pr = abi.decode(
            _deliveryData.payload,
            (ProposalResult)
        );

        proposals[pr.proposalId].isAccepted = pr.isAccepted;
    }
